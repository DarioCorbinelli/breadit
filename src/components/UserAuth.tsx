...
const UserAuth: FC<UserAuthProps> = ({ className, type, redirectUrl, ...props }) => {
  return (
    <div className={cn('max-w-sm mx-auto flex flex-col items-center text-center gap-4', className)} {...props}>
      ...
      <UserAuthForm className='mt-2' redirectUrl={redirectUrl} />
      {type === 'signIn' ? (
        <p className='text-sm text-muted-foreground mt-2'>
          Non hai ancora un account?{' '}
          <Link href={'/sign-up' + (redirectUrl ? `?redirectUrl=${redirectUrl}` : "")} className='underline underline-offset-2'>
            Registrati
          </Link>
        </p>
      ) : (
        <p className='text-sm text-muted-foreground mt-2'>
          Hai già un account?{' '}
          <Link href={`/sign-in` + (redirectUrl ? `?redirectUrl=${redirectUrl}` : "")} className='underline underline-offset-2'>
            Accedi
          </Link>
        </p>
      )}
